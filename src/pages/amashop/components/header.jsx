...
const Header = () => {
  ...
  const ATagMaker = ({name, path, icon}) => {
    return (
      <Link key={name} onClick={() => setActive(path)} to={path} className='flex lg:px-4 px-2 font-bold min-w-max gap-1 text-[16px]'>
        <span>
          {icon}
        </span>
        {name}
  ...
  const LiMaker = ({ename, name, path, icon, activeIcon, i}) => {
    return (
      <li key={name} onClick={() => setActive(path)} className={`relative flex justify-center items-center ${i== '1' ? 'h-full' : 'h-8'}  xl:px-3 px-2`}>
      ...
      <div className='flex flex-col w-full max-w-[1600px]'>
        <div className='flex justify-between h-16'>
          <div className='flex items-center justify-start md:w-3/4 w-full lg:gap-8 gap-4'>
            ...
            <div className='md:hidden flex items-center justify-start md:w-5/6 w-full max-w-[600px] h-12 pr-10 bg-g_Background_Shaded_Shop rounded-[10px] text-[18px] cursor-pointer'>
              ...
              <span to='/shop' className='absolute right-32 md:hidden flex font-primary text-2xl font-[700]'>
                AMASHOP
              </span>
            </div>
          </div>
        ...
        <nav className={`${styles.paddingX} text-gray-500 fixed bottom-0 left-0 md:hidden flex flex-row items-center w-screen h-16 justify-between border-t-[3px] bg-white z-50`}>
          {navLinkPhone.map((link) => {
            return (
              <li key={link.name} onClick={() => setActive(link.path)} className='relative flex justify-center items-center h-full xl:px-3 px-2'>
                <Link to={link.path} className='flex flex-col gap-2  font-sans text-[14px] min-w-max'> 
                  <span className='flex items-center justify-center pt-[3px] scale-[1.7]'>
                    {active.substring(active.length - 4) == 'shop' && link.ename == 'shop' ? link.activeIcon
                    : active.includes(link.ename) && link.ename !== 'shop' ? link.activeIcon : link.icon}
                  </span>
                  {link.name}
                </Link>
                {link.ename == 'card' && <span className='absolute right-3 bottom-6 flex items-center justify-center w-2 h-2 p-[.7rem] rounded-full bg-red-500 text-g_Text_White'>
                    {data.cards.length}
                  </span>
                }
              </li>
