...
const Comment = ({post, handleSetComment, targetUser}) => {
  ...
  return (
    <div className='fixed top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-gray-400/25 py-4 lg:px-10 md:px-6 px-4 z-40'>
      <div className='flex flex-col w-full h-full max-h-[800px] max-w-[700px] bg-white overflow-y-scroll rounded-[.6rem]'>
        <header className='sticky top-0 bg-white flex justify-center items-center w-full px-4 min-h-[70px] text-[1.3rem] gap-3 border-b-[1px] z-30'>
          <span>{targetUser && (targetUser.firstName + ' ' + targetUser.lastName)}</span>
